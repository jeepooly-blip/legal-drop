...
const ROLE_HOME: Record<UserRole, string> = {
  client: '/dashboard', lawyer: '/onboarding', admin: '/admin',
}
...
cookies: {
  ...
  if (user) {
    ...
    if (path === '/login' || path === '/register') {
      return NextResponse.redirect(new URL(role ? ROLE_HOME[role] : '/dashboard', req.url))
    }
    if (path.startsWith('/admin')      && role !== 'admin')  return NextResponse.redirect(new URL('/dashboard', req.url))
    if (path.startsWith('/whiteboard') && role !== 'lawyer') return NextResponse.redirect(new URL('/dashboard', req.url))
    if (path.startsWith('/dashboard')  && role === 'lawyer') return NextResponse.redirect(new URL('/whiteboard', req.url))
  }
  return res
}
export const config = {
